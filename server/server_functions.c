...
#define MAX_GAME_X + (uint16_t)(pow(2,13)-201) //max x location
#define MAX_GAME_Y + (uint16_t)(pow(2,13)-201) //max y location
...
void eventEatObject(Player *pPla, Object *pObj){
	int k = GROWTH_FACTOR;
	pPla->scale += ceil(k*OBJ_SIZE/pPla->scale);
...
void eventEatPlayer(Player *eater, Player *eaten){
    ...
    eater->scale += ceil(k*eaten->scale/eater->scale);
...
void sendPoints(Game *game, char *buf, int socket, socklen_t addrlen, int type){
	Player *pPla = game->sPlayers;
	int sent = 0;
	int plLength;
